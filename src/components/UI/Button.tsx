...
const Button = ({ children, variant = 'standard', ...props }: ButtonProps) => {
	const getStyling = () => {
		switch (variant) {
			case 'ghost':
				return 'inline-flex w-full cursor-pointer justify-center rounded-md bg-background px-3 py-2 text-sm font-semibold text-text shadow-xs inset-ring-1 inset-ring-ring hover:bg-hover sm:col-start-1 sm:mt-0'
			case 'standard':
			default:
