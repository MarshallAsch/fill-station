...
const ListBox = ({
	...
	return (
		<Listbox value={selected} onChange={handleChange}>
			<Label className='text-text block text-sm/6 font-medium'>{title}</Label>
			<div className=''>
				...
				<ListboxOptions
					transition
					className='bg-background absolute z-60 mt-1 max-h-60 w-full max-w-100 overflow-auto rounded-md py-1 text-base shadow-lg outline-1 outline-black/5 data-leave:transition data-leave:duration-100 data-leave:ease-in data-closed:data-leave:opacity-0 sm:text-sm'
				>
					{items.map((item, index) => (
						<ListboxOption
							key={item.id || index}
							value={item}
							className='group text-text data-focus:bg-accent data-focus:text-white-text relative cursor-default py-2 pr-9 pl-3 select-none data-focus:outline-hidden'
						>
							<div className='flex'>
							...
							<span className='text-accent-text absolute inset-y-0 right-0 flex items-center pr-4 group-not-data-selected:hidden group-data-focus:text-white'>
